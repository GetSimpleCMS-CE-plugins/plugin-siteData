...
this.optionElement = "input";
    ...
    <button @click="active = 'data'" style="background:red;border-radius:5px;border:none;padding:5px 10px;color:#fff;">Data 📰</button>
    <button @click="active = 'settings'" style="background:black;border-radius:5px;border:none;padding:5px 10px;color:#fff;">Settings ⚙️</button>



    <div class="Data" x-show="active == 'data'"
        ...
        <template x-for="data in datas" :key="data.id+data.option">
            <div>
                ...
                <input type="text" x-value="data.data" x-show="data.option=='input'" x-model="data.data"
                    style="width:100%;padding:7px;margin-top:10px;background:rgba(0,0,0,0.1);border:none;">

                <textarea x-show="data.option=='textarea'" x-model="data.data" x-html="data.data.replace('\u0027',`'`)"
                    style="width:100%;padding:7px;margin-top:10px;background:rgba(0,0,0,0.1);border:none;"
                    rows="5"></textarea>
            </div>
        </template>
    ...
    <div class="Settings" x-show="active == 'settings'"
        ...
        <button @click="pushNew" class="addnew"
            style="background:green;color:#fff;border:none;border-radius:5px;padding:0.5rem 1rem;margin-right:0;">Add New</button>


        <template x-for="(data,index) in datas" :key="data.title+data.option">
            <div
                style="width:100%;padding:0.5rem;border:solid 1px #ddd;display:grid;grid-template-columns:1fr 1fr 300px 90px;gap:5px;margin:5px 0">

                <b x-text="data.title"></b>
               
                <i x-text="'type input: '+data.option"></i>

                <code style="border:solid 1px #ddd;text-align:center">
                [% siteData=<span x-text="data.id"></span> %]     
             <br>
                &lt;?php siteData("<span x-text="data.id"></span>");?&gt;
                </code>
                <button style="background:red;color:#fff;border-radius:5px;border:none" @click="
...
">Delete🗑️</button>
    ...
    <form method="post">
        ...
        <input type="submit" name="saver" value="Save Inputs 💾" style="background:green;border-radius:5px;border:none;padding:5px 10px;color:#fff;margin-top:20px;">
    </form>
