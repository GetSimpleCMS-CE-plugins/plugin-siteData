		...
		<div id="Settings" class="w3-container w3-margin-top mytabs" style="display:none">
			...
			<template x-for="(data, index) in datas" :key="data.title+data.option">
				<tr x-sort:item="index" style="display:grid;grid-template-columns:1fr 1fr 2fr 1fr;width:100%;">
					...
					<td>
						<code class="w3-codespan w3-tiny w3-round" onclick="copyToClipboard(this)">[% siteData=<span x-text="data.id"></span> %]</code>
						<br>
					...
					<td style="display:flex;align-items:center;gap:5px;height:50px">
						<button @click="datas.splice(index, 1);" class="w3-btn w3-round w3-tiny w3-red"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24"><rect width="15" height="15" fill="none" /><path fill="#fff" d="M7 21q-.825 0-1.412-.587T5 19V6H4V4h5V3h6v1h5v2h-1v13q0 .825-.587 1.413T17 21zm2-4h2V8H9zm4 0h2V8h-2z" /></svg></button>
							
						<button @click="moveElement(datas, index, index-1);" class="w3-btn w3-round w3-tiny w3-green" style="margin:0 !important"> <svg fill="#fff" width="15px" height="15px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg"><title>Up</title><path d="M11.25 15.688l-7.656 7.656-3.594-3.688 11.063-11.094 11.344 11.344-3.5 3.5z"></path></svg></button>
						
						<button @click="moveElement(datas, index, index+1);" class="w3-btn w3-tiny w3-green w3-round w3-margin-bottom" style="margin:0 !important"> 
						<svg fill="#fff" width="15px" height="15px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg"><title>Down</title><path d="M11.125 16.313l7.688-7.688 3.594 3.719-11.094 11.063-11.313-11.313 3.5-3.531z"></path></svg>
						</button>
					</td>
				</tr>
			</template>
