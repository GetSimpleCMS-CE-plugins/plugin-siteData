...
<div class="test" x-data='{
	...
	<div class="Settings" x-show="active == 'settings'"
		...
		<button @click="pushNew" class="addnew" style="background:green;color:#fff;border:none;border-radius:5px;padding:0.5rem 1rem;margin-right:0;">Add New</button>
		...
		<template x-for="(data,index) in datas" :key="data.title+data.option">
			<div style="width:100%;padding:0.5rem;border:solid 1px #ddd;display:grid;grid-template-columns:1fr 1fr 300px 90px;gap:5px;margin:5px 0">
				...
				<button style="background:red;color:#fff;border-radius:5px;border:none" @click="datas.splice(index, 1);">Delete🗑️</button>
			</div>
		</template>
	</div>
